...
module.exports.authenticate = function (req, res, next) {
  ...
  jwt.verify(token, config.jwt.secret, function (err) {
    ...
    if (err && err.name === 'TokenExpiredError')
      return res.status(401).send({ code: 'TokenExpiredError', message: 'Token expired.' })
...
module.exports.generateJWT = function (email, validity) {
  let payload = { email: email },
    token = jwt.sign(payload, config.jwt.secret, { expiresIn: validity });
  return token;
}